...
sigNoise = 0.5    # stDev of gaussian random noise added to initialized spins
...
def deltaE(g, x, y, phi):
    nbrs = (np.array([x, y]) + [[0, 1], [0, -1], [1, 0], [-1, 0]]) % L

    dE = 0
    for nbr in nbrs:
        dE += np.cos(g[x, y] - g[nbr[0], nbr[1]])
        dE -= np.cos(g[x, y] - g[nbr[0], nbr[1]] + phi)
...
if save:
    ...
    path = os.path.join(fileDir, 'Data_XY')
    pathLK = os.path.join(fileDir, 'Data_XY/L=%s_K=%s' % (L, K))
    pathLKT = os.path.join(fileDir, 'Data_XY/L=%s_K=%s/T=%s' % (L, K, T))
    filename = os.path.join(fileDir, 'Data_XY/L=%s_K=%s/T=%s/%s.txt'
                            % (L, K, T, int(time.time())))
